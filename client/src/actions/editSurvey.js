...
export const normalizeSurvey = (survey) => {
  ...
  let questionOrder = survey.questions.map(question => question._id)
  return {
    id: survey.id,
    title: survey.title,
...
export const activeQuestion = questionId => dispatch => {
  ...
  if (questionId === 'header') { dispatch(switchTab(tabTypes.EDIT_SURVEY_TAB)) } else { dispatch(switchTab(tabTypes.EDIT_QUESTION_TAB)) }
}
